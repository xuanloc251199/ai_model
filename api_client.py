...
def upload_image_to_git(image_path: str):
    """
    Đẩy ảnh lên GitHub sử dụng các lệnh git: git add . và git commit --amend.
    """
    try:
        # Chạy lệnh git add để thêm file vào index
        subprocess.run(["git", "add", "."], check=True)
        
        # Commit các thay đổi và sửa lại thông điệp commit
        subprocess.run(["git", "commit", "--amend", "--no-edit", "-m", "Add or update image"], check=True)
        
        # Đẩy thay đổi lên GitHub với lệnh push -f (force push)
